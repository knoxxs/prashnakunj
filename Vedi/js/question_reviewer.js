...
function quesDisplay()
{
	...
	jQuery.get(
					...
					function(data)
					{
						...
						jQuery('#quesAnswer').after('<br><div id="respond"><form id="commentform"><h5>Post answer</h5><textarea name="comment" id="new_answer" style="height: 30px; width: 650px; border: 1px dotted rgb(72,72,72);" tabindex="4"></textarea><p><input name="submit" id="submit" tabindex="5" value="Post" onClick="ansPost()"/></p><input type="hidden" name="comment_post_ID" value="" id="comment_post_ID" /><input type="hidden" name="comment_parent" id="comment_parent" value="0" />	</form></div><div class="clearfix"></div>');
	...
	jQuery.post(
			...
			function(data)
			{
				if(data.head.status == 200)
				{
					...
					alert("Comment was not posted. Try again!");
			}, 
			"json"	
		);
}
	...
	jQuery.post(
			...
			function(data)
			{
				if(data.head.status == 200)
				{
					...
					alert("Suggestion was not posted.");
			}, 
			"json"	
		);
}
function ansPost()
{
	var x = jQuery('#new_answer').val();
	result = {};
	result['QID']=getCookie('questionID');
	result['string']=x;
	jQuery.post(
			"/qcorner/question/answer/post",
			result,
			function(data)
			{
				if(data.head.status == 200)
				{
					...
					alert("There was some error. Answer was not posted!");
			}, 
			"json"	
		);
}
