...
class User extends Base{ 
	...
	public function addTag($tag){
		$db = $this->getDb();
		$db->query("SELECT * FROM Subscribe WHERE userName='$this->userName' AND tagName='$tag'");
		if($db->returned_rows == 0){
			$db->query("SELECT * FROM Tags WHERE name='$tag'");
			if($db->returned_rows == 0){
				$db->query("INSERT INTO Tags VALUES('$tag', '$this->userName')");
			}
			return $db->query("INSERT INTO Subscribe VALUES('$this->userName','$tag')");
		}else{
			return false;
		}
	}
